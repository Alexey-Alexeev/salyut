...
export const DELIVERY_CONSTANTS = {
    ...
    // Адрес самовывоза
    PICKUP_ADDRESS: {
        fullAddress: 'Рассветная улица, 4, деревня Чёрное, городской округ Балашиха, Московская область, 143921',
...
export function isFixedDeliveryCity(city: string): boolean {
    ...
    return FIXED_DELIVERY_CITIES.some(fixedCity =>
...
export function calculateDeliveryCost(city: string, distanceFromMKAD?: number): number {
    ...
    if (distanceFromMKAD !== undefined) {
        if (distanceFromMKAD <= 0) {
            ...
            return DELIVERY_CONSTANTS.MIN_DELIVERY_COST + (DELIVERY_CONSTANTS.COST_PER_KM * distanceFromMKAD)
        }
    }
 ...
 */
export function calculateDelivery(data: DeliveryData): DeliveryCalculationResult {
...
export function extractCityFromAddress(address: string): string {
    ...
    const sortedCities = [...MOSCOW_OBLAST_CITIES].sort((a, b) => b.length - a.length)
