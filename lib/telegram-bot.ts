...
export class TelegramOrderBot {
  ...
  async handleUpdate(update: TelegramUpdate) {
    ...
    if (!this.isAuthorized(message.from.id)) {
      await this.sendMessage(message.chat.id, '❌ У вас нет доступа к этому боту')
      return
    }
    ...
    try {
      ...
      await this.sendMessage(message.chat.id, '❌ Произошла ошибка при обработке команды')
    }
  }
  ...
  private async sendHelp(chatId: number) {
    const helpText = `
🎆 *КупитьСалюты - Управление заказами*
...
💡 *Совет:* Используйте кнопки в уведомлениях о новых заказах для быстрого управления!
  ...
  private async sendRecentOrders(chatId: number) {
    try {
      ...
      for (const order of recentOrders) {
        ...
        const date = order.created_at ? new Date(order.created_at).toLocaleDateString('ru-RU') : 'Н/Д'
  ...
  private async sendStats(chatId: number) {
    try {
      ...
      const stats = ordersByStatus.reduce((acc, order) => {
  ...
  private async sendOrderDetails(chatId: number, orderId: string) {
    try {
      ...
      const itemsText = items
        .map(item => `• ${item.product_name || 'Товар'} - ${item.quantity} шт. × ${item.price_at_time.toLocaleString('ru-RU')} ₽`)
        .join('\n')

      const contactInfo = order.contact_method && order.customer_contact
        ? `\n📱 ${order.contact_method === 'telegram' ? 'Telegram' : 'WhatsApp'}: ${order.customer_contact}`
        : ''

      const createdAt = order.created_at ? new Date(order.created_at).toLocaleString('ru-RU') : 'Н/Д'
...
\`/cancel_${order.id}\` - Отменить
  ...
  private async updateOrderStatus(chatId: number, orderId: string, newStatus: string) {
    try {
      ...
      if (!updated) {
        await this.sendMessage(chatId, `❌ Заказ #${orderId.slice(0, 8)} не найден`)
        return
      }
