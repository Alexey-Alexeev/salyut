...
export const PriceRangeFilter = React.memo<PriceRangeFilterProps>(({ 
  ...
  }, [fromValue, toValue, maxPrice, onPriceChange, onApplyFilter, onMobileFilterClose])

  // Обработчики ввода только обновляют локальное состояние
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-xs text-muted-foreground">
        {(fromValue || toValue) ? (
          <>
            {fromValue || minPrice} ₽ — {toValue ? formatPrice(parseInt(toValue) || maxPrice) : formatPrice(maxPrice)} ₽
          </>
        ) : (
          <>
