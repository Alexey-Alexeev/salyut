...
import { getCityInPrepositionalCase, isMoscowRegionCity, normalizeCityName } from '@/lib/cities';
...
export async function generateMetadata({ params }: CityPageProps): Promise<Metadata> {
    ...
    const { moscowRegionCities } = await import('@/lib/cities');
    if (!moscowRegionCities.includes(normalizedCity)) {
        return {
            title: 'Город не найден | СалютГрад',
            description: 'Запрашиваемый город не найден в списке обслуживаемых городов.',
        };
    }

    const cityInPrepositionalCase = getCityInPrepositionalCase(normalizedCity);
    ...
    return {
        ...
        openGraph: {
            ...
            description: `Купить салюты и фейерверки в ${cityInPrepositionalCase} с доставкой. Профессиональный запуск салютов в ${cityInPrepositionalCase}. Качественная пиротехника для праздников.`,
            url: `https://салютград.рф/${normalizedCity.toLowerCase()}`,
            siteName: 'СалютГрад',
            locale: 'ru_RU',
            type: 'website',
            images: [
        ...
        alternates: {
            canonical: `https://салютград.рф/${normalizedCity.toLowerCase()}`,
        },
        robots: {
            index: true,
            follow: true,
...
export async function generateStaticParams() {
    // Импортируем список городов для генерации статических параметров
    const { moscowRegionCities } = await import('@/lib/cities');
    const params = moscowRegionCities.slice(0, 20).map(city => ({
        city: city.toLowerCase(),
    }));
    console.log('Generated static params:', params.slice(0, 5));
    return params;
}
...
export default async function CityPage({ params }: CityPageProps) {
    ...
    // Проверяем, есть ли город в списке (сравниваем с нормализованным названием)
    const { moscowRegionCities } = await import('@/lib/cities');

    console.log('Debug - cityName:', cityName);
    console.log('Debug - normalizedCity:', normalizedCity);
    console.log('Debug - moscowRegionCities includes:', moscowRegionCities.includes(normalizedCity));
    console.log('Debug - first few cities:', moscowRegionCities.slice(0, 5));
            ...
            <section className="relative flex min-h-[50vh] items-center justify-center overflow-hidden py-12 sm:py-16 md:min-h-[70vh]">
                <div className="absolute inset-0 z-0">
                    <Image
                        ...
                        alt={`Красивые фейерверки и салюты в ${cityInPrepositionalCase} - СалютГрад`}
                        fill
                ...
                <div className="relative z-10 mx-auto max-w-4xl space-y-4 px-4 text-center">
                    <h1 className="rounded-lg bg-black/40 p-4 text-4xl font-bold leading-tight text-white md:text-6xl">
                        Незабываемые <span className="text-orange-400">салюты в {cityInPrepositionalCase}</span> для ваших праздников
                    </h1>

                    <p className="mx-auto max-w-2xl rounded-lg bg-black/30 p-4 text-lg text-white md:text-xl">
                        Качественная пиротехника с доставкой в {cityInPrepositionalCase}. Создайте магию праздника вместе с нами!
                    </p>
