...
async function testConnection() {
  ...
  console.log(
    'DATABASE_URL:',
    process.env.DATABASE_URL ? '✅ Установлен' : '❌ Не найден'
  );
  console.log(
    'NEXT_PUBLIC_SUPABASE_URL:',
    process.env.NEXT_PUBLIC_SUPABASE_URL ? '✅ Установлен' : '❌ Не найден'
  );
  console.log(
    'NEXT_PUBLIC_SUPABASE_ANON_KEY:',
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY ? '✅ Установлен' : '❌ Не найден'
  );
  console.log(
    'TELEGRAM_BOT_TOKEN:',
    process.env.TELEGRAM_BOT_TOKEN ? '✅ Установлен' : '❌ Не найден'
  );
  console.log(
    'TELEGRAM_CHAT_ID:',
    process.env.TELEGRAM_CHAT_ID ? '✅ Установлен' : '❌ Не найден'
  );
  ...
  try {
    ...
    const result =
      await client`SELECT version(), current_database(), current_user`;
    ...
    for (const expectedTable of expectedTables) {
      ...
      console.log(
        `  ${expectedTable}: ${exists ? '✅ Существует' : '❌ Отсутствует'}`
      );
    }
    ...
    try {
      ...
      if (parseInt(ordersCount[0].count) > 0) {
        ...
        recentOrders.forEach(order => {
          console.log(
            `    - ${order.id.slice(0, 8)}: ${order.customer_name} (${order.created_at})`
          );
        });
      }
    } catch (error) {
    ...
    try {
      const productsCount =
        await client`SELECT COUNT(*) as count FROM products`;
      console.log(`  Количество товаров: ${productsCount[0].count}`);
    } catch (error) {
      console.error(
        '  ❌ Ошибка при проверке таблицы products:',
        error.message
      );
    }
